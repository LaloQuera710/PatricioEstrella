...
read -p "[*] Elige una opcion: " opc1
	case $opc1 in
			1 )	clear
                ...
                echo "##############################################"
                echo
                 ifconfig -a | awk '/^[a-z]/ { iface=$1; } /inet addr:/ { next; } /^[[:space:]]*ether/ { print iface,$2; }'
                echo
                echo "##############################################"
                echo "[##] Modos compatibles con la Tarjeta de Red: "
                echo "##############################################"
                echo
                 iw list | awk '/Supported interface modes/,/Band/{if(NR>1)print}' | head -n -1
                echo
                echo "##############################################"
                echo "[##] Modo Tajeta de Red (Monitor/Manager): "
                echo "##############################################"
                echo
                 iwconfig | grep -oP '^\S+' | xargs -n1 sh -c 'echo -n $0" Mode:"; iwconfig $0 | awk -F "\"" "/Mode/ {print $2}"'
                echo
                echo "##############################################"
                echo "[##] Tasa de Transferencia de Datos: "
                echo "##############################################"
                echo
                 iw list | grep -oP "VHT TX highest supported:\s*\K\d+" | awk '{print "Tasa de transferencia: " $1 " Mbps"}'
                echo
				;;
            2 )	clear
                logo2
                    echo -e "$white"
                 iw list > requisitos/resultados/frecuencias.txt
                sleep 2
